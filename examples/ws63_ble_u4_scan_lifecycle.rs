...
fn run(mut parts: hisi_rf::ws63::RadioParts) -> ! {
    for cycle in 0..3 {
        ...
        if cycle == 1 {
            ...
            let result = firmware::wait_with_runner(scanner.stop(), || progress(&mut parts));
            if result.is_err() {
                firmware::fail(b"RFDBG_RADIO_U4_BLE_STOP_ERR\r\n");
            }
        }
    }
    firmware::log(b"RFDBG_RADIO_U4_BLE_SCAN_LIFECYCLE_OK\r\n");
    loop {
        progress(&mut parts);
        firmware::drive_scheduler();
    }
